<template>
  <section class="intro-section zen-canvas" ref="rootEl">
    <!-- 视差生长的竹子背景 -->
...
import BambooParallax from "../effects/BambooParallax.vue";
...
const handleScroll = () => {
  ...
  // top 从 wh 减小到 0 时，progress 从 0 增长到 1
  bambooProgress.value = Math.max(0, Math.min(1, (wh - rect.top) / wh));
};
...
<style scoped>
/* Animations (shared with sibling sections) */
.reveal-up {
  opacity: 0;
