  ...
  <div class="bamboo-forest" ref="forestRef">
    <svg class="bamboo-svg" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
      ...
      >
        ...
        <!-- 叶子 -->
        <g v-for="(leaf, lKey) in bamboo.leaves" :key="'l'+lKey" :transform="leaf.transform" :opacity="leaf.opacity">
...
function generateForest() {
  ...
  for (let i = 0; i < bambooCount; i++) {
    ...
    const leftSide = joints.map(j => `${j.x - j.width/2},${j.y}`).join(' L ');
    const rightSide = joints.slice().reverse().map(j => `${j.x + j.width/2},${j.y}`).join(' L ');
    const trunkPath = `M ${leftSide} L ${rightSide} Z`;
    
    const branchSegments: BambooBranchSegment[] = [];
    const leaves: BambooLeaf[] = [];

    function addLeafCluster(lx: number, ly: number, branchAngleDeg: number, branchDepth: number) {
      ...
      for(let k = 0; k < leafCount; k++) {
        ...
        leaves.push({
          transform: `translate(${lx + ox}, ${ly + oy}) rotate(${svgRot}) scale(${scale})`,
          opacity: Math.min(1, Math.max(0.1, leafOpacity))
        });
      }
    }

    function buildBranchTree(bx: number, by: number, angleDeg: number, length: number, width: number, level: number, branchDepth: number) {
      if (level > 3) return;
      
      let numSegments = 0;
      if (level === 1) numSegments = 4 + Math.floor(rnd() * 2); // 一级枝条分节变多（像主干一样有节）
      ...
      for (let s = 0; s < numSegments; s++) {
        ...
        branchSegments.push({ x1: cx, y1: cy, x2: nx, y2: ny, width: curWidth });
      ...
      for (let s = 1; s < numSegments; s++) {
        if (level === 1) {
          ...
          if (rnd() < symProb) {
            ...
            buildBranchTree(parentJointsX[s]!, parentJointsY[s]!, angle1, childLen, childWidth, 2, branchDepth);
            buildBranchTree(parentJointsX[s]!, parentJointsY[s]!, angle2, childLen, childWidth, 2, branchDepth);
          } else {
            // 单侧不对称的二级分支
            const side = rnd() > 0.5 ? 1 : -1;
            const childAngle = curAngle + side * (30 + rnd() * 20);
            buildBranchTree(parentJointsX[s]!, parentJointsY[s]!, childAngle, childLen, childWidth, 2, branchDepth);
          }
        } else if (level === 2) {
          // 二级分支有概率直接变成3级分支
          if (rnd() < 0.7) {
            ...
            if (rnd() < symProb) {
              ...
              buildBranchTree(parentJointsX[s]!, parentJointsY[s]!, angle1, childLen, childWidth, 3, branchDepth);
              buildBranchTree(parentJointsX[s]!, parentJointsY[s]!, angle2, childLen, childWidth, 3, branchDepth);
            } else {
              const side = rnd() > 0.5 ? 1 : -1;
              const childAngle = curAngle + side * (20 + rnd() * 20);
              buildBranchTree(parentJointsX[s]!, parentJointsY[s]!, childAngle, childLen, childWidth, 3, branchDepth);
            }
          }
        }
      }
    ...
    for (let jIdx = 1; jIdx < numJoints; jIdx++) {
      ...
      if (jIdx <= maxBranchJoints) {
        ...
        const startSides = rnd() < symProbRoot ? [1, -1] : [rnd() > 0.5 ? 1 : -1];

        for (const side of startSides) {
          const angleDeg = 30 + rnd() * 20;
          // 为了迎合弯曲的主干，整体角度向弯曲一侧偏转
          const baseBranchAngle = side * angleDeg + (leanDirection * 20 * Math.pow(1 - depth, 2));

          // 越到顶端一代分支越长，甚至比三节竹子长（通过深度反比增加基础长度）
          const maxL = 150 + (1 - depth) * 200 + rnd() * 50;
          const branchBaseWidth = Math.max(1.5, joint.width * (0.3 + rnd() * 0.2));

          buildBranchTree(joint.x + side * (joint.width / 2), joint.y, baseBranchAngle, maxL, branchBaseWidth, 1, depth);
        }
      }
    }
    ...
    for (let topB = 0; topB < 2 + rnd() * 2; topB++) {
      ...
      buildBranchTree(topJoint.x, startY, angle, topBranchLength, branchBaseWidth, 1, 0); // 顶端深度强制为 0
    }
